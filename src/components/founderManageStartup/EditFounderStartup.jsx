...
export default function EditFounderStartup({ updateState, data }) {
  ...
  const handleImageUpload = async () => {
    try {
      ...
      toast.danger("Image updation failed!");
    } finally {
      setIsUploading(false);
    }
  };
  useEffect(() => {
    if (updateState.isOpen && data) {
      reset({
        startup_name: data?.startup_name || "",
        industry: data?.industry || "",
        founding_stage: data?.founding_stage || "",
  ...
  const handleOnSubmit = async (formData) => {
    if (isNewImageSelected && !imageUrl) {
      toast.danger("Please upload the selected image first!");
      return;
    }
    try {
      ...
      const response = await axios.put(
        `${baseURL}/api/founder/update_startup/${_id}`,
        startupFormData,
      );
      ...
      if (data.success) {
        ...
        router.push("/dashboard/founder/manage_startup");
      } else {
  ...
  return (
    <Modal isOpen={updateState.isOpen} onOpenChange={updateState.setOpen}>
      <Modal.Backdrop>
        <Modal.Container>
          <Modal.Dialog className="rounded-xs w-full xs:max-w-md bg-linear-to-r from-orange-200 via-orange-100 to-orange-50">
            <Modal.CloseTrigger />
            <Modal.Body>
              <div className="flex flex-col justify-center items-center gap-2">
                {/* Wrapper */}
                <div className="text-center flex flex-col gap-1">
                  <h1 className="font-bold text-2xl">Edit Your Startup</h1>
                  <p>
                    Update your startup details to keep your profile accurate
                    and attractive to investors, mentors, and collaborators.
                  </p>
                </div>
                <div>
                  {/* Main container */}
                  <form
                    className="shadow-sm shadow-orange-500 p-4 bg-orange-300 flex flex-col gap-2 w-full rounded-sm"
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {isNewImageSelected && !imageUrl ? (
                        <p className="text-red-500">
                          Upload required for new image
                        </p>
                      ) : (
                        <p className="text-green-500">Image ready</p>
                      )}
                    </div>
                    <div className="flex flex-col gap-1">
                      <div className="flex flex-col gap-1">
